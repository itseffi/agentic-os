...
Open question: items 12 and 13 are design calls, not mechanical fixes. They need a decision
...
Fix by guarding the whole goals section, from the header at `setup.sh:145` through the
write ending at `setup.sh:296`, so the interview is skipped rather than the answers being
collected and then discarded:

```bash
if [ -f "GOALS.md" ]; then
    print_info "File exists: GOALS.md (preserving your version)"
    print_info "Delete or rename it and re-run setup to regenerate."
else
    # existing questions and cat > GOALS.md
fi
```
...
9. Run `setup.sh` twice in the same directory. Expect `GOALS.md` from the first run to
   survive.
